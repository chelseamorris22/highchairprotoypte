...
function Shortlist({
  ...
  const shortlistOrder = ["stokke-clikk", "maxi-cosi-moa", "lalo-chair", "stokke-tripp-trapp"];
  const shortlist = shortlistOrder
    .map((id) => products.find((product) => product.id === id))
    .filter((product): product is Product => {
      if (!product) return false;
      return ["Best option", "Good with caveats"].includes(product.priority);
    });
